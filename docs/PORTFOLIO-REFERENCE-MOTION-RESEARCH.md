...
| Project workflow diagram | Open a project evidence disclosure | A few times per visit | Explanation and state | Reveal the diagram content, then run one signal through its nodes in reading order. Leave the completed diagram static | 240 ms reveal; 1,100 ms signal; 90 ms node offsets | `cubic-bezier(0.23, 1, 0.32, 1)` reveal; `cubic-bezier(0.77, 0, 0.175, 1)` signal | `transform`, `opacity` | Show the completed diagram immediately | Do not animate disclosure height. Replay only on an explicit close-and-open action, not on scroll. Keep labels readable throughout |
...
- No constantly moving marquee, technology ticker, cursor follower, or looping
  workflow signal.
...
| Infinite workflow pulse | It reinforces the systems motif | Continuous movement competes with diagram labels. A single event-driven traversal explains the same idea |
