...
Leo's personal portfolio site, and **the current live one: leosanga.vercel.app**.
This is the canonical portfolio Leo links to now; the older `portfolio-site/`
(static HTML/CSS/JS at leosanga.github.io/portfolio) is superseded and kept only
for its chat/scenario widgets. Originally generated by [Lovable](https://lovable.dev), then
...
When work touches identity, color, typography, the workflow mark, imagery,
social graphics, diagrams, or other Leo Sanga branded output, first read
`../leo-sanga-brand/AGENTS.md` and follow its read order (goal, current state,
approved playbook, relevant guide under `docs/applications/`). Exact values come
from `brand/tokens.json`; approved files and provenance come from
`brand/assets/manifest.json`. This repo keeps authority over its implementation,
content evidence, testing, release process, and whether to adopt a brand change.
Consulting the brand repo never authorizes a portfolio change or deployment by
itself.
...
- This is Leo's live portfolio (leosanga.vercel.app); expect
